...
b_tn = np.load(f'global/global stim b_tn revised.npy')
pg_tn = np.load(f'global/global stim p_tn revised.npy')
b_tp = np.load(f'global/global stim b_tp revised.npy')
pg_tp = np.load(f'global/global stim p_tp revised.npy')

bl = np.load(f'local/local stim b revised.npy')
pl = np.load(f'local/local stim p revised.npy')
pl_tn = np.load(f'local/local stim p_tn revised.npy')
pl_tp = np.load(f'local/local stim p_tp revised.npy')
...
pl_tn, pl_tnci = mean_and_ci(pl_tn, 95)
pg_tp, pg_tpci = mean_and_ci(pg_tp, 95)
...
axs[0].errorbar(binsizes-2, impg, yerr=impgci, fmt='none',
                elinewidth=2, ecolor='#004D40', capsize=5, alpha=0.3)
axs[0].errorbar(binsizes+2, impl, yerr=implci, fmt='none',
...
axs[1].scatter(binsizes, b_tn, color='#D81B60', s=50,
               marker='s', alpha=0.5, label='Baseline')
axs[1].scatter(binsizes-2, pg_tn, color='#004D40',
               s=50, marker='o', label='Optogenetic')
axs[1].scatter(binsizes+2, pl_tn, color='#1E88E5',
               s=50, marker='^', label='Electrical')
axs[1].errorbar(binsizes-2, pg_tn, yerr=pg_tnci, fmt='none',
                elinewidth=2, ecolor='#004D40', capsize=5, alpha=0.3)
axs[1].errorbar(binsizes+2, pl_tn, yerr=pl_tnci, fmt='none',
                elinewidth=2, ecolor='#1E88E5', capsize=5, alpha=0.3)
...
axs[2].errorbar(binsizes-2, pg_tp, yerr=pg_tpci, fmt='none',
                elinewidth=2, ecolor='#004D40', capsize=5, alpha=0.3)
axs[2].errorbar(binsizes+2, pl_tp, yerr=pl_tpci, fmt='none',
                elinewidth=2, ecolor='#1E88E5', capsize=5, alpha=0.3)
